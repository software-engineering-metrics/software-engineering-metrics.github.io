...
cannot do for an engineering organisation.

Measurement exists to answer questions an organisation cannot otherwise
...
organisations need metrics to demonstrate to legislatures and the public that
...
This distinction recurs constantly through this book and is formalised in the
...
In large organisations, the riskiest gap is not a bad metric, it is an area
...
letting them stay invisible by default. An organisation that knows what it is
...
4. **If we deleted this metric tomorrow, who would notice, and what would
   ...
   purely through inertia. For a large organisation with dozens of team
...
(chapter 1.4) is not optional at this scale. Standardise definitions across
...
**Enterprise.** A global insurance company's engineering organisation had
...
organisation defined a shared, decision-driven core of DORA metrics
...
modernisation programme. The team's existing metrics were entirely internal
...
The return on deliberate measurement is decision quality. An organisation
...
did not. An organisation relying on anecdote cannot do any of that with
...
to the cost of the alternative, which is a large organisation making
...
- **Level 2, Develop:** A basic set of metrics exists for some teams, mostly
  ...
  a decision.
- **Level 3, Standardise:** Every tracked metric has a documented purpose and
...
- **Level 5, Orchestrate:** Measurement is a living capability: the
  organisation routinely identifies its own blind spots, tests whether its
...
3. Where in our organisation has a diagnostic metric quietly become evaluative?

...
For large organisations the stakes are higher because the distance between
...
- **Assume every incentivised metric will be gamed.** Design against it from
...
- **A guardrail is not optional on an incentivised metric.** Every metric you
...
particular direction? If yes, it is an incentivised metric and needs a
...
### Pair every incentivised metric with a guardrail
...
visibly expensive: a team that improves the incentivised number by degrading
...
recognisable shapes. **Threshold gaming** optimises right up to a target and
...
| Heavily incentivised metrics | Strong, fast behavioural response | High distortion risk, often within one reporting cycle |
...
game cheaply, and by pairing anything you do incentivise with a guardrail
...
1. **For each metric anyone's reward depends on, what is the cheapest way to
   ...
   design the exploit for every incentivised number on your dashboard: how
   ...
   you are not ready to incentivise the metric yet. This exercise is
...
3. **Does every incentivised metric on our dashboard have a paired
   ...
   round of damage to trust. Audit your incentivised metrics for this pairing
   specifically.
...
5. **Have we ever removed an incentive from a metric after discovering it was
   being gamed, and what did that cost us in trust to fix?** Organisations
...
management later and turned into a KPI nobody who built it would recognise.
...
organisation that designs guardrails up front spends a modest amount of extra
effort defining a second metric alongside the first. An organisation that
...
- **Publishing an incentivised metric with no guardrail:** the single most
...
- **Level 1, Initiate:** Metrics are incentivised ad hoc, with no
  consideration of gaming risk, and distortion is discovered only after
  quality or trust visibly suffers.
- **Level 2, Develop:** Some teams recognise gaming after the fact and adjust
  informally, but there is no consistent practice of designing guardrails
  in advance.
- **Level 3, Standardise:** Every incentivised metric organisation-wide
...
- **Level 4, Manage:** Gaming risk is actively monitored: definitions are
  ...
  own right.
- **Level 5, Orchestrate:** The organisation treats Goodhart's law as a
...
1. What is the most consequential metric in our organisation that has no guardrail today?
...
4. Which of the four gaming patterns (threshold, definition, timing, substitution) is our organisation most prone to?
...
- Prefer **ratios, rates, and cohorts** over raw counts wherever they capture
  the same behaviour.
- Every incentivised metric needs a **guardrail**, designed at the same time,

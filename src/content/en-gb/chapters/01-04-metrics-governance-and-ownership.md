...
solvable with the same discipline organisations already apply to code:
...
programme survive contact with organisational scale. A single team can keep
its metric definitions in someone's head and correct drift through daily
conversation. An organisation with dozens of teams, each producing and
...
For enterprise and government organisations, governance carries additional
...
understand, is what keeps an organisation's numbers meaning the same thing
...
| Lightweight charter per metric set | Cheap, readable, scales with the organisation | Requires discipline to keep current; can be skipped under deadline pressure |
...
1. **Does every metric that crosses a team boundary have a named owner, and
   would that owner recognise themselves as accountable if asked today?**
...
2. **Where do we currently compute the same nominally named metric two
   ...
   governance failures in large organisations, and it is entirely
...
3. **When did we last retire a metric, and what triggered that decision?**
   An organisation that can only describe how it adds metrics, never how it
...
**Enterprise.** This is where governance earns its keep. Standardise
...
- **Level 2, Develop:** Some teams write informal documentation for their
  ...
  consistency.
- **Level 3, Standardise:** Every metric crossing a team boundary has a
  documented charter, a named owner, and a single agreed source of truth,
  enforced organisation-wide.
...
- **Level 5, Orchestrate:** Governance is proportional to consequence,
  ...
  unowned metrics), and the organisation can demonstrate, on demand, the full
...
5. Who owns our organisation's single most consequential public-facing metric, by name?
...
- *Measuring and Managing Performance in Organizations*, by Robert D. Austin
  (organisational dysfunction around metric ownership and use).

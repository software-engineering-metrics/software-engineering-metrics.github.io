...
that teaches an organisation something true from one that teaches it
...
after a major reorganisation, can shape resourcing decisions for years based
...
time-based metrics, and be sceptical of any "up 40% this month" headline
computed from a small underlying count.

**Enterprise.** Statistical mistakes at this scale get baked into resourcing
and reorganisation decisions that affect hundreds of people. Invest in
...
a budget justification can have outsised real-world consequences and
...
improvement in on-time performance for a newly digitised scheduling system,
...
The return on statistical literacy is avoided misdirection: an organisation
that correctly attributes an improvement, or correctly recognises noise as
...
- **Level 2, Develop:** Some analysts apply medians or percentiles
  informally, but there is no consistent organisational practice and
  confounds are rarely checked.
- **Level 3, Standardise:** Medians and percentiles are the default for
  skewed time-based metrics; sample sizes are shown alongside
  percentage-based metrics organisation-wide.
...
- **Level 5, Orchestrate:** Statistical rigor is built into the tooling
  ...
  the organisation can demonstrate that a specific past decision was

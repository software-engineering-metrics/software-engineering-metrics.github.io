...
organisations run dozens of such queues at once, shared code review pools,
...
- **Utilisation does not scale linearly with wait time.** As a shared
  resource approaches full utilisation, queueing delay grows sharply, not
...
### Track utilisation directly for every shared, capacity-constrained resource
...
one with the worst combination of high utilisation and high failure or
skip rate, not the one that happens to be easiest to instrument.

### Set staffing and WIP limits with utilisation in mind, not just throughput
...
utilisation on average has effectively infinite wait time in practice,
...
| Utilisation tracking on shared resources without deeper modelling | Simple, actionable, catches the single biggest cause of runaway wait times | Says nothing about why utilisation is high or what to do about the underlying cause |
...
than no model. Little's law and basic utilisation tracking give up some
precision but require no specialised statistical skill and fit directly
...
case where a single shared resource, a large CI fleet, a specialised
...
2. **Which shared resources in our delivery pipeline are running close to
   full utilisation, and do we actually know their utilisation number?**
   Most teams can name a resource that "always feels busy" but have never
   measured its utilisation directly. Identify the two or three most
...
4. **Where in our pipeline is the true bottleneck, the slowest stage that
   ...
   worst combination of high utilisation and high failure or skip rate.
...
5. **If we added capacity to our most constrained shared resource, would
   ...
   a WIP limit, or a change to how work is prioritised before it enters the
   queue.
...
utilisation problem worth naming even without any formal model behind it.

**Small business.** A small business team rarely needs anything more
sophisticated than tracking utilisation on its one or two genuinely shared
...
utilisation tracking earns its keep, because a single overloaded shared
...
rate, capacity, utilisation, is frequently the clearest evidence available
...
individual team had changed how it worked. A utilisation analysis found
...
scheduling policy so no single team's burst of activity could monopolise
...
caseworker capacity sized to keep utilisation below 85%, now tracked as a
...
The total cost of adoption is genuinely low. Little's law and utilisation
...
each other and periodically reviewing utilisation on shared resources
before they become the organisation's next unexplained lead-time
regression.

## Anti-patterns and pitfalls

- **Sizing a shared resource's capacity to match its average arrival rate
  exactly:** guarantees high utilisation and runaway wait times whenever
...
- **Treating "our people are always busy" as a compliment:** it is a
  symptom of high utilisation, the leading cause of long, unpredictable
  wait times.
...
- **Level 2, Develop:** Arrival rate, work in process, and cycle time are
  ...
  law or against utilisation on shared resources.
- **Level 3, Standardise:** Little's law is a routine consistency check
  across delivery pipelines, and utilisation is tracked explicitly for the
...
- **Level 4, Manage:** Success, failure, and skip rate are tracked
  ...
  utilisation targets, not just average demand.
- **Level 5, Orchestrate:** The organisation models its major pipelines as
...
2. Name the single shared resource in our organisation that most people would agree is "always busy," and find its actual utilisation number.
...
- **Wait time grows sharply, not gradually, as utilisation approaches full
...
- Model a multi-stage pipeline as a **queue of queues**, and invest in the
  stage with the worst combination of high utilisation and high failure or
  skip rate, not the stage that is easiest to improve.
- Favour cheap, adoptable checks, **Little's law and utilisation

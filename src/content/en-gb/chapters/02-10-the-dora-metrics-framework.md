...
practices correlate with organisational performance. The result was four
...
part's organising framework. That placement is not a rejection of the
...
computed consistently from pipeline and incident data lets an organisation
...
comparisons. Enterprise organisations still use it to prioritise platform
...
| DORA as this part's sole organising metric set | Simple, familiar to most engineering leaders | Misses the value-mix question entirely, this book's reason for deprioritising it here |
...
3. **Has anyone in our organisation used a DORA metric in an individual
...
**Government.** DORA metrics still give a modernisation programme a
...
**Enterprise.** A large telecommunications company's platform modernisation
...
new customer value versus rework. The engineering organisation had no
...
**Government.** A state government's IT modernisation office adopted DORA
...
- **Level 2, Develop:** Some teams instrument DORA from the pipeline, but
  ...
  against.
- **Level 3, Standardise:** All four DORA metrics are instrumented
...
- **Level 4, Manage:** DORA and flow metrics are reviewed together as a
  standard pairing at every level of the organisation, and DORA is never
  used for individual evaluation.
- **Level 5, Orchestrate:** The organisation can point to specific cases

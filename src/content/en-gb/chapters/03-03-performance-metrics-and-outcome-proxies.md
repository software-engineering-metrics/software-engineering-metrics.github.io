...
rewards visible busyness. Enterprise organisations comparing performance
across many teams need signals that resist gaming through raw output volume;
government organisations justifying technology investment to oversight
...
| Multiple converging signals | More trustworthy, resists single-metric gaming | Harder to summarise in one number; requires more context to interpret |
...
the damage from false precision compounds across the whole organisation's
...
decisions: an organisation that can see which teams' work genuinely moves
...
outcome) and resisting organisational pressure to collapse the picture back
...
- **Collapsing multiple converging signals back into one falsely precise
  number under organisational pressure:** loses the honesty the multi-signal
...
- **Level 3, Standardise:** Performance is measured at the team level using
...
- **Level 5, Orchestrate:** Performance data directly informs resourcing and
  investment decisions, and the organisation can point to specific

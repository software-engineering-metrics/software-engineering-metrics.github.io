...
tracked metrics in this entire book, cheap to compute, easy to visualise as
...
For large teams, coverage targets are often adopted organisation-wide as a
quality gate, precisely the kind of incentivised, high-visibility metric
chapter 1.2 warns is most exposed to gaming. Enterprise and government
organisations that set a blanket coverage percentage requirement without a
paired effectiveness check are, in effect, incentivising exactly the
...
- **Coverage is more useful as a floor than as a target to maximise.** A
...
### Use coverage to find untested code, not as a target to maximise
...
### Prioritise coverage and mutation testing on critical paths first
...
incentivised number. Resolve the tension by accepting the added complexity
of mutation testing and risk-based prioritisation as the cost of an honest
...
**Enterprise.** Blanket, organisation-wide coverage targets are a common
and consequential mistake at this scale, since they incentivise exactly the
...
- **Level 2, Develop:** A coverage target exists and is tracked, but no
  mutation testing or risk-based prioritisation informs how effort is
  allocated.
- **Level 3, Standardise:** Coverage floors are enforced consistently in
  CI, with risk-based prioritisation directing where coverage effort
  concentrates.
...
- **Level 5, Orchestrate:** The organisation can point to specific defect
  reductions traced to mutation-testing-informed prioritisation, and

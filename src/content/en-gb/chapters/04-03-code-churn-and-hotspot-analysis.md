...
**Hotspot analysis**, popularised by Adam Tornhill's work on software
...
This lets a team or organisation identify, with real evidence rather than
...
and government organisations managing large, long-lived codebases depend on
this data-driven prioritisation to direct genuinely scarce refactoring
...
- **A hotspot is a prioritisation signal, not an automatic verdict.** Human
...
- **This analysis scales precisely where intuition fails**: in large
  codebases too big for any individual to survey and prioritise by feel
  alone.
...
right combination of signals to prioritise by.
...
### Use hotspot data to inform, not replace, team-level prioritisation
conversations

Present hotspot analysis as evidence in a prioritisation discussion, not as
...
deprioritise a known hotspot temporarily, an upcoming planned rewrite makes
...
| Approach | Pros | Cons |
| --- | --- | --- |
| Intuition-based prioritisation | Fast, no tooling required, leverages team's contextual knowledge | Skewed by recency, personal preference, and whoever complains loudest |
...
| Hotspot analysis cross-referenced with incident data | Validated, strongest evidence for prioritisation | Requires reliable incident-to-code linkage, which not every organisation has |

The central tension is **evidence versus context**. Hotspot analysis
provides objective, scalable evidence that intuition-based prioritisation
...
analysis as the evidence base for a prioritisation conversation, combined
...
5. **Are we currently prioritising refactoring work based on evidence, or
   based on whoever complained most recently or most loudly?** Be honest
   about your team's actual current prioritisation process and how it
...
6. **What would it cost us, in defect rate or delivery slowdown, to leave
   ...
   forces a concrete cost estimate that can anchor a prioritisation
   decision, rather than leaving the hotspot as an abstract, easily
   deprioritised concern.
...
**Enterprise.** Hotspot analysis is where evidence-based prioritisation
...
also a persuasive, concrete tool for justifying modernisation investment to
...
troublesome, but no formal prioritisation had ever followed from those
...
underwent a hotspot analysis as part of a modernisation business case. The
...
modernising," became the centerpiece of a successful budget request for a
targeted, incremental modernisation effort focused specifically on that
...
- **Prioritising refactoring based on the loudest complaint rather than
...
- **Level 2, Develop:** Some teams informally check churn or complexity
  data, but there is no consistent, organisation-wide hotspot analysis
  practice.
- **Level 3, Standardise:** Hotspot analysis combining churn and complexity
  runs regularly and consistently informs refactoring prioritisation
  organisation-wide.
...
- Treat a hotspot ranking as **evidence for prioritisation**, not an

...
rather than leaving it as a vague, perpetually deprioritised worry that
...
a single, prioritised, visible backlog that competes fairly for investment
...
which compounds further. Enterprise and government organisations
...
quantified, prioritised debt backlog, is the mechanism that lets an
organisation actually manage the trade-off deliberately instead of drifting
into crisis.
...
- **Technical debt is a deliberate metaphor for a manageable trade-off, not
  ...
  business decision.
- **Unmeasured debt loses the prioritisation competition against feature
...
### Prioritise using impact, not age or loudest advocate
...
most active development. Resist prioritising by which item has been on the
...
that decision explicitly and move the item to a deliberately deprioritised
...
| Approach | Pros | Cons |
| --- | --- | --- |
| No formal debt tracking | No overhead | Debt loses the prioritisation competition by default; compounds invisibly |
...
structural pressure for debt to lose every individual prioritisation
...
4. **Is our debt backlog prioritised by genuine business impact, or by
   whichever item has been raised most persistently or has sat there
   longest?** Cross-reference your current prioritisation against churn
...
5. **What debt items should we explicitly accept as permanent, rather than
   ...
   discuss moving it to an explicitly deprioritised status.
...
here, since the individual prioritisation competition between debt and
feature work reliably favours features across dozens of teams
simultaneously without a structural counterweight. Standardise debt
quantification practice organisation-wide so debt items can be compared
...
modernisation budget to oversight bodies, since it turns a vague "the
...
components, produced a quantified, prioritised backlog that directly
supported a multi-year modernisation budget request. Critically, the
...
- **No visible, tracked debt backlog:** debt loses the prioritisation
...
- **Prioritising debt by age or advocacy volume rather than impact:**
...
- **Letting debt sit indefinitely on an active backlog without ever
  ...
  happen and clutters genuine prioritisation.
...
- **Level 3, Standardise:** A visible, quantified debt backlog exists
  organisation-wide, with protected remediation capacity allocated
  consistently.
- **Level 4, Manage:** Debt items are prioritised by measured impact
...
- Technical debt is a **manageable trade-off, not a shameful secret**;
  quantify it rather than leaving it as a vague, perpetually deprioritised
  concern.
- **Quantify cost to fix versus cost to carry** for each item so it
  competes fairly against feature work.
- **Prioritise by impact** (carrying cost combined with churn), not by age
  or advocacy volume.
...
- *Your Code as a Crime Scene*, by Adam Tornhill (hotspot analysis as an
  input to debt prioritisation, chapter 4.3).

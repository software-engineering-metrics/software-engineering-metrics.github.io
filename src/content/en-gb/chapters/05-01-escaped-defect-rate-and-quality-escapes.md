...
5 as a whole is concerned with. Enterprise organisations use it to justify
investment in the testing and review practices from Part 4; government
organisations, where an escaped defect can mean an incorrect benefit
...
- **Root-cause categorisation turns a count into a diagnostic tool.**
...
### Standardise classification criteria across teams
...
| Standardised, audited classification | Fair, comparable, resists gaming | Requires ongoing governance and periodic auditing effort |
...
cannot be fairly compared or aggregated at an organisational level, and
creates a quiet incentive for a team to classify generously to protect its
own metrics. Resolve the tension by investing in standardised, documented
...
2. **Would two different teams classify the same defect's severity the same
   way, or has classification drifted apart across the organisation?** Pick
...
organisation-wide quality comparison meaningless. Invest in documented,
...
prioritised for remediation because no defect had yet occurred to make the
...
- **Level 3, Standardise:** Severity classification is standardised and
  documented organisation-wide, with root-cause categorisation applied
  consistently.

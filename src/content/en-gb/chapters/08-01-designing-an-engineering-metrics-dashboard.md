...
choosing what to show whom, visualising it honestly, and structuring the
...
principles throughout: every visualisation choice either helps or hinders a
...
organisations running dozens of team dashboards need consistency without
rigidity, shared standards that still allow each audience's specific needs
to be met; government organisations, whose dashboards may face public
scrutiny or serve as the basis for oversight reporting, need the honest
visualisation standards this chapter recommends applied with particular
...
- **Design for a specific audience and decision, not for comprehensive
  ...
  well.
- **Every visualisation choice either helps or actively misleads.** Apply
...
- **Guardrail pairs belong on the same view.** Never separate an
  incentivised metric from its guardrail onto different dashboards or
  different sections.
...
### Apply honest visualisation standards consistently
...
skewed data, and annotate context (deploys, incidents, organisational
...
on-call load data (chapter 6.3), is a small design choice with an outsised
...
2. **Do any of our dashboards separate an incentivised metric from its
...
3. **Would our dashboard's visualisations pass chapter 1.6's honest
   visualisation standards: zero-based axes, trend over snapshot, medians
...
visualisation standards and the explicit not-for-evaluation statement even
...
dozens of team dashboards need enough shared standard (honest visualisation
...
particular rigor in honest visualisation and explicit governance
...
this chapter's guardrail-pairing and honest visualisation standards,
...
dashboard had been criticised publicly for a chart showing "average"
processing time using a truncated y-axis that visually exaggerated a modest
improvement, a violation of chapter 1.6's honest visualisation standards
...
- **A single dashboard trying to serve every audience:** usually serves no
  one well.
- **Separating an incentivised metric from its guardrail across different
  views:** recreates the incentive-exposure risk chapter 1.2 warns against.
- **Dishonest visualisation choices:** truncated axes, cherry-picked date
...
- **Level 1, Initiate:** A single, uncurated dashboard, if any, serves all
  audiences poorly, with no honest visualisation standard or guardrail
  pairing.
- **Level 2, Develop:** Some audience-specific views exist, but visualisation
  standards are inconsistent and ownership is unclear.
- **Level 3, Standardise:** Audience-specific dashboards with consistent,
  honest visualisation standards and guardrail pairing are established
  organisation-wide, each with a named owner.
- **Level 4, Manage:** Dashboards are reviewed on a regular cadence, with
  ...
  are actively pruned.
- **Level 5, Orchestrate:** The organisation's dashboard design practice is
  a trusted, well-governed capability, and the organisation can point to
...
3. Would our current charts pass an honest visualisation audit?
...
- Apply **honest visualisation standards** (chapter 1.6) as hard
  requirements: zero-based axes, trend over snapshot, medians over
  averages for skewed data.
- **Never separate an incentivised metric from its guardrail** across
...
- *The Visual Display of Quantitative Information*, by Edward R. Tufte
  (the foundational text on honest, high-integrity data visualisation).

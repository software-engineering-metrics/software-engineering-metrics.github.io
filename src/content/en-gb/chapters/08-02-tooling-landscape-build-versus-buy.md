...
Every organisation implementing this book's guidance eventually faces a
...
cost-benefit analysis specific to your organisation's scale, existing data
...
many organisations toward buying at least the foundational layer, but it
has not eliminated the build option's genuine advantages for specific,
customised needs, particularly around the outcome telemetry chapter 7.4
argues is now the necessary center of a metrics programme, which is
frequently the least standardised, most organisation-specific category of
measurement this book covers.

For large teams, this decision has real, ongoing budget and engineering
capacity consequences. Enterprise organisations often need to integrate
...
government organisations frequently face procurement constraints and data
...
- **This is rarely an all-or-nothing decision.** Most mature metrics
  programmes combine bought tooling for well-standardised metrics with
  built tooling for organisation-specific outcome telemetry.
- **Buy for well-standardised, widely needed metrics; build for
  genuinely organisation-specific ones.** DORA metrics and pull request
...
organisations below a certain scale, since building equivalent
...
### Build for genuinely organisation-specific outcome telemetry
...
tooling is far less standardised and often cannot capture your organisation's
specific business logic and data model without extensive, expensive
customisation that may end up costing more than building the equivalent
...
data [lock-in](https://en.wikipedia.org/wiki/Vendor_lock-in) is a durable organisational risk, not merely an inconvenience,
...
For government and regulated enterprise organisations, data sovereignty
...
| Approach | Pros | Cons |
| --- | --- | --- |
| Buy commercial tooling | Fast to deploy, mature feature set, vendor-maintained | Less customisable for organisation-specific outcome metrics; potential lock-in |
| Build internal tooling | Fully customised, full data ownership and control | Significant, ongoing engineering investment; duplicates effort for commodity metrics |
| Hybrid: buy commodity layer, build outcome layer | Balances cost efficiency with genuine customisation where it matters most | Requires integration work to connect bought and built components coherently |
| Buy everything, including outcome telemetry, via extensive vendor customisation | Single vendor relationship, potentially simpler procurement | Can become as expensive as building, with less ultimate control over the result |

The central tension is **customisation need versus development cost**. The
metrics that most benefit from customisation, outcome telemetry tied
...
the ones where genuine customisation matters least. Resolve the tension by
matching the decision to this pattern directly: buy where standardisation
...
1. **For each metric family this book covers, would we genuinely benefit
   from customisation, or would a standardised commercial tool serve us
...
2. **Have we evaluated our current or prospective vendor's data export and
   ...
   often discovered only when an organisation actually tries to switch.
...
well and are priced accessibly for smaller organisations; buying the
commodity layer is almost always the right choice, and building anything
custom is rarely justified until your organisation has grown considerably
...
building the organisation-specific outcome telemetry layer, since your
...
and still lagged behind mature commercial offerings for the standardised
...
customisation cost of forcing a genuinely organisation-specific need into
...
- **Building custom tooling for well-standardised, commodity metrics:**
  duplicates engineering effort many vendors have already invested in
  heavily.
- **Buying commercial tooling for genuinely organisation-specific outcome
  telemetry without checking fit first:** risks expensive, ill-fitting
  customisation or an unmet need.
...
- **Treating this as a single, all-or-nothing decision:** misses the hybrid
  approach that best matches most organisations' actual, mixed needs.
...
- **Level 2, Develop:** Some analysis occurs, but integration cost is
  ...
  considered.
- **Level 3, Standardise:** A deliberate, hybrid build-versus-buy strategy
  matches commodity metrics to bought tooling and organisation-specific
...
- **Level 5, Orchestrate:** The organisation's tooling landscape reflects a
  mature, deliberate hybrid strategy, regularly reviewed as commercial
  offerings and organisational needs evolve, with demonstrated value from
  both the bought and built components.

## Ideas for discussion

1. Which of our current metrics would benefit most from customisation we are not currently getting?
...
- This is rarely all-or-nothing; most mature programmes **combine bought
  tooling for commodity metrics with built tooling for organisation-specific
  outcome telemetry**.
- **Buy for standardised metrics** (DORA, review analytics, survey
  infrastructure); **build for genuinely organisation-specific** outcome
  measurement.
...
- **Procurement, security, and sovereignty constraints** can override a
  pure cost-benefit calculation, particularly for government organisations.
...
- U.S. Federal Risk and Authorisation Management Program (FedRAMP)

...
- Published the book in four locales under `locales/`: `en-gb-oxendict`
  ...
  (American English). `en-001`, `en-gb`, and `en-us` are mechanically
  derived from `en-gb-oxendict` by the new `tools/localize.py`; see
  `spec/locales.md`. `docs/` no longer exists; every reference to it across
  `spec/`, `AGENTS.md`, `tests/validate.py`, `tools/gen_nav.py`, and
  `tools/stats.py` now points at `locales/<locale>/`.
...
- Renamed Part 2 from "Delivery and Flow Metrics" to "Flow Metrics" and
  reorganised it around Mik Kersten's Flow Framework. Added four new

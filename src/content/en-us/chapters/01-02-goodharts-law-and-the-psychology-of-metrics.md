...
the metric's designer and the person whose behavior it shapes grows with
...
- **Ratios and ranges resist gaming better than raw counts.** A raw count
  rewards volume; a well-chosen ratio rewards the actual behavior you want.
...
rewards the underlying behavior instead of the volume. A **cohort**, a group
...
the same underlying behavior, choose the rate.
...
| Ratios and rates | Reward the right behavior, resist volume gaming | Can hide a shrinking denominator problem |
...
| Heavily incentivized metrics | Strong, fast behavioral response | High distortion risk, often within one reporting cycle |

The central tension is **motivational power versus distortion risk**. The
metrics that move behavior fastest, tying a number directly to reward, are
...
4. **How far does this metric travel from the person who understands its
   intent before it reaches the person whose behavior it shapes?** A metric
...
5. **Have we ever removed an incentive from a metric after discovering it was
   ...
   distorted behavior, and the repair costs more than prevention would have.
...
gamed behavior and rebuilding trust in the number afterward. The retail
...
behavior for months before anyone notices. Every chapter after this one
...
- **Distance** between a metric's designer and the person whose behavior it

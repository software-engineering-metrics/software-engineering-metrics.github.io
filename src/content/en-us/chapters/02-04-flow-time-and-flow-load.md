...
many concurrent strategic initiatives and government programs running
...
**Government.** Multi-year programs routinely accumulate large, implicit
...
directly, showing that the program's own flow-time growth is
...
program had accumulated flow load across dozens of parallel workstreams
without any single tracked total, each workstream sponsor believing their
own initiative was appropriately resourced in isolation. A program
office analysis using Little's law showed that the program's aggregate
...
year. The program adopted an explicit flow-load ceiling, and new

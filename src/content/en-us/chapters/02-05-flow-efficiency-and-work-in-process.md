...
**Government.** Multi-year programs often accumulate enormous implicit
...
program had accumulated over forty concurrent workstreams across its
portfolio, each with its own sponsor and its own justification, with no
single view of total work in process. A program-level flow-efficiency
...
cross-agency sign-off. The program introduced explicit portfolio-level WIP

...
metrics program that actually improves outcomes from one that merely
...
**Government.** A national tax agency's engineering program needed to
...
report story-point or milestone completion alone, the program presented a
...
simple "delivered on schedule" report from a prior program had failed to
do the year before.
...
1. What single number are we currently using as a performance proxy that we should retire in favor of a converging set?

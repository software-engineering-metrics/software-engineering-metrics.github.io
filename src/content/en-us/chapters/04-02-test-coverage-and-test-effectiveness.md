...
"kills" (fails against) most mutants is genuinely verifying behavior; a
...
| Coverage plus mutation testing | Verifies tests actually check behavior, resists gaming | More computationally expensive; requires tooling investment |
...
| Risk-based, critical-path-first coverage | Concentrates effort where it matters most | Requires judgment to correctly identify genuinely critical paths |
...
without meaningfully asserting on their behavior, purely to satisfy the

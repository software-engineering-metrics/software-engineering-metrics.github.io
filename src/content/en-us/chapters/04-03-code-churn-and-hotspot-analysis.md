...
subjective judgment to find its targets. [Version control](https://en.wikipedia.org/wiki/Version_control) history already
...
- **A hotspot is a prioritization signal, not an automatic verdict.** Human
  judgment is still needed to decide what action a specific hotspot
  warrants.
...
### Investigate the top hotspots with human judgment before acting
...
file that legitimately needs frequent change because it sits at the center
...
an automatic mandate that overrides a team's own contextual judgment about
...
| Churn combined with complexity (hotspot analysis) | Strong, evidence-based, automatic from existing data | Requires combining two data sources and interpreting results with judgment |
...
it lacks the contextual judgment a team has about why a given hotspot
...
with, never substituting for, the team's own contextual judgment about
...
3. **For our top hotspot right now, is the underlying problem essential
   ...
   together and make this judgment explicitly rather than assuming either
   answer.
...
the periodic analysis effort and the human judgment time to interpret
...
- **Treating a hotspot ranking as an automatic action list with no human
  judgment:** misses the essential-versus-accidental distinction that
...
- Treat a hotspot ranking as **evidence for prioritization**, not an
  automatic verdict; human judgment is still required.
...
- *Software Design X-Rays*, by Adam Tornhill (further techniques for
  behavioral code analysis using version control history).

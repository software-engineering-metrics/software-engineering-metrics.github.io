...
**Government.** A national digital identity program needed to demonstrate
value to a legislative committee skeptical of the program's ongoing cost.
Rather than reporting delivery metrics (modules shipped, on schedule), the
program reported citizen outcome metrics directly: median time to
...
the program had used in previous budget cycles, and directly supported

...
organizing principle a metrics program has to be built around. This is
...
consequences for how a metrics program should be built and rebuilt going
...
programs it is embedded in, should hold both to the same outcome-telemetry
...
- **A metrics program built primarily around output metrics is now a
...
your metrics program against the next technology shift as much as the
current one.
...
2. **What specific outcome-telemetry infrastructure investment have we
   been deferring in favor of faster, cheaper output tracking?** Name a
...
5. **If our AI tooling vendor or the current generation of AI coding
   assistants changed dramatically next year, would our metrics program
...
metrics program outcome-first rather than needing to unwind an existing
...
**Government.** Government technology programs evaluated primarily on
...
engineering metrics program from scratch specifically because its
...
credibility with its oversight committee that its predecessor program,
...
program that remains trustworthy and credible through the current
...
of a metrics program that becomes progressively less trustworthy as
...
- **Level 5, Orchestrate:** The organization's metrics program is
...
- Build this investment as a **durable capability**, independent of any
  specific AI tool or vendor, protecting your metrics program against
...
- *Measure What Matters*, by John Doerr (outcome-oriented goal setting as
  an organizing principle for a metrics program, the model this chapter

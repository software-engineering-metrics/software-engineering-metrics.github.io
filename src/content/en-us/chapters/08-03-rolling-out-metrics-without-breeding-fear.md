...
untrustworthy metrics program if the rollout itself teaches engineers
...
- **Early rollout incidents set lasting expectations.** The first few
  ...
  program gets perceived going forward.
...
trust in the entire program going forward; a fast, transparent correction
...
metrics program than a system that quietly polices for gaming from
...
1. **Was our current metrics program's purpose and explicit non-goals
...
3. **Has our metrics program sustained genuinely diagnostic-only use over
...
4. **Has a metric ever been misused punitively, even once, even
   ...
   program far more than the original incident itself.
...
thing" with visible skepticism long after the original program had been
quietly shelved. A new, deliberately relaunched program explicitly
...
acknowledgment of past failure, rather than simply relaunching as if the
...
- **Rushing to evaluative use before a genuine diagnostic-only trust period
  has elapsed:** the single most common way a new metrics program
  provokes gaming behavior immediately.
...
- **Relaunching a previously mishandled metrics program without
...
- **Level 3, Standardize:** New metrics programs are consistently rolled
...
1. Was our current metrics program's purpose communicated before or after concerns arose?
...
- **Respond to the first mishandled incident immediately and visibly**;
  silence confirms the exact fear driving gaming behavior.
...
- *Drive: The Surprising Truth About What Motivates Us*, by Daniel H. Pink
  ...
  corrupts metric-driven behavior).

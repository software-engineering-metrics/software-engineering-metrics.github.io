...
individual DORA metric maturity while still being at Level 1 on program
...
program maturity is realistically the minimum, not the average, across
...
business units, and government organizations reporting program maturity
...
- **Program maturity is the minimum across its dimensions, not the
...
more flattering composite. A program with excellent instrumentation
(Level 4) but weak cultural trust (Level 1) is not, in any meaningful
sense, a Level 2 or 3 program; the weak dimension actively undermines the
...
This consolidated model answers "how mature is our overall program";
...
reassess program maturity on a regular cadence, annually is common,
rather than only after a crisis (a discovered gaming incident, a
credibility-damaging public report) forces the question. A program that
...
| Using only chapter-level models | Detailed, metric-specific guidance | Misses the cross-cutting view of overall program health |
...
constraint on your program's overall trustworthiness and value. Resolve
the tension in favor of honesty: report the minimum, and use both this
...
2. **What is our weakest dimension, and does that match our intuition about
   our program's overall health, or does it reveal something we had not
...
4. **When did we last formally reassess our overall program maturity, and
...
consolidated model, is a genuinely useful artifact for demonstrating
program rigor to an oversight body, provided the assessment is
...
program's value, since even excellent instrumentation was producing data
...
correctly identifying the actual constraint on a metrics program's value,
rather than investing further in an already-strong dimension while a weak
one continues to undermine the whole program's trustworthiness, exactly
...
where it will genuinely move the overall program's maturity, rather than
...
continues to quietly corrupt the value of everything else the program has
built.

## Anti-patterns and pitfalls

- **Averaging dimension scores into a more flattering composite:** hides
  the real constraint on the program's overall value.
...
- **Level 1, Initiate:** No formal cross-cutting assessment exists;
  ...
  program health is unexamined.
- **Level 2, Develop:** Some informal awareness of overall program
...
- Program maturity spans five cross-cutting dimensions: **governance,
...
- *Measuring and Managing Performance in Organizations*, by Robert D.
  Austin (organizational assessment of metrics program health and
  dysfunction).
